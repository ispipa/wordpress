...
class VikBookingAdminWidgetLatestFromGuests extends VikBookingAdminWidget
{
	...
	public function loadActivities()
	{
		...
		$vcm_lib = VCM_SITE_PATH . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'lib.vikchannelmanager.php';
		if (!is_file($vcm_lib)) {
			echo 'e4j.error.missing VCM';
			return;
		}
		if (!class_exists('VikChannelManager')) {
			require_once $vcm_lib;
		}
		if (!method_exists('VikChannelManager', 'getLatestFromGuests')) {
			// VCM is outdated
			echo 'e4j.error.VCM is outdated';
			return;
		}
	...
	public function render($data = null)
	{
		...
		$vcm_lib = VCM_SITE_PATH . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'lib.vikchannelmanager.php';
		if (!is_file($vcm_lib) || !$vbo_auth_bookings) {
			return;
		}
		if (!class_exists('VikChannelManager')) {
			require_once $vcm_lib;
