		...
		<div class="vbo-modal-overlay-content-body vbo-modal-overlay-content-body-scroll">
			<div class="vbo-modal-widgets-newcontainer vbo-modal-widgets-forms" style="display: none;">
				<div class="vbo-modal-widgets-form-data-fields">
					...
					<div class="vbo-modal-widgets-form-data-field vbo-modal-widgets-form-data-field-save">
			...
			<div class="vbo-modal-widgets-newwidget vbo-modal-widgets-forms" style="display: none;">
				<div class="vbo-modal-widgets-form-data-fields">
					<div class="vbo-modal-widgets-form-data-field">
						<label for="vbo-newwidget-id"><?php echo JText::translate('VBO_WIDGETS_SELWIDGADD'); ?></label>
						<select id="vbo-newwidget-id" onchange="vboWidgetSetNewDescr(this.value);">
							<option value=""></option>
						<?php
		...
		if ($widgets_welcome) {
			...
			<div class="vbo-widgets-welcome-wrap vbo-modal-widgets-forms" style="display: none;">
				<div class="vbo-widgets-welcome-inner">
					...
					<div class="vbo-widgets-welcome-actions">
						<div class="vbo-widgets-welcome-action">
...
<script type="text/javascript">
	 ...
	 */
	function vboDoAjax(url, data, success, failure, attempt) {
		...
		return jQuery.ajax({
			...
			if (attempt < VBO_AJAX_MAX_ATTEMPTS && vboIsConnectionLostError(err)) {
				...
				setTimeout(function() {
					...
					vboDoAjax(url, data, success, failure, (attempt + 1));
				}, 500);
			} else {
	 ...
	 */
	function vboThrottleEvent(method, delay) {
		var time = Date.now();
		return function() {
	...
	jQuery(document).ready(function() {
		 ...
		 */
		document.addEventListener('vbo-admin-widgets-savemap', vboDebounceEvent(vboHandleMapSaving, 2000));
	...
	function vboWidgetsAddWidget(elem) {
		...
		jQuery('#vbo-newwidget-id').val('').trigger('change');
		// set modal title
	...
	function vboWidgetSetNewDescr(widget_id) {
		...
		jQuery('.vbo-newwidget-descr').html('').hide();
		if (!widget_id.length) {
			return;
		}
		...
		for (var i in vbo_admin_widgets_names) {
			...
			if (vbo_admin_widgets_names[i]['id'] == widget_id) {
				jQuery('.vbo-newwidget-descr').html(vbo_admin_widgets_names[i]['descr']).fadeIn();
				break;
			}
		}
	}
	 ...
	 */
	function vboWidgetsAddWidgetToDoc() {
		if (!vbo_admin_widgets_last_container || !vbo_admin_widgets_last_container.length) {
			// parent container not found
		...
		var widget_id = jQuery('#vbo-newwidget-id').val();
		var widget_descr = jQuery('.vbo-newwidget-descr').html();
		var widget_name = jQuery('#vbo-newwidget-id').find('option:selected').text();
