...
class VikBookingViewPrices extends JViewVikBooking {
	...
	function display($tpl = null) {
		...
		if ($wizard) {
			if ($rplanpub1) {
				$q = "INSERT INTO `#__vikbooking_prices` (`name`, `idiva`, `breakfast_included`, `free_cancellation`) VALUES (" . $dbo->quote(JText::translate('VBOSTANDARDRATE')) . ", 0,  $rplanbk1, 1);";
				$dbo->setQuery($q);
				$dbo->execute();
			}
			if ($rplanpub2) {
				$q = "INSERT INTO `#__vikbooking_prices` (`name`, `idiva`, `breakfast_included`, `free_cancellation`, `canc_deadline`) VALUES (" . $dbo->quote(JText::translate('VBONONREFRATE')) . ", 0, $rplanbk2, 0, 7);";
				$dbo->setQuery($q);
				$dbo->execute();
			}
