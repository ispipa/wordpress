		...
		if (!empty($pcheckindate) && !empty($pcheckoutdate)) {
			if (VikBooking::dateIsValid($pcheckindate) && VikBooking::dateIsValid($pcheckoutdate)) {
				...
				if ($second > $first) {
					...
					if ($forcebooking || ($room_available && $room_notlocked)) {
						$forced_reason = $forcebooking && (!$room_available || !$room_notlocked) ? JText::translate('VBO_FORCED_BOOKDATES') : $forced_reason;
						// Customer
