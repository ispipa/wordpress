...
<form action="index.php?option=com_vikbooking" method="post" name="adminForm" id="adminForm">
						...
						?>
							<div class="vbo-roomdet-guests-toggle-cont">
								<span tabindex="0" class="vbo-roomdet-guests-toggle <?php echo $count_pax_data >= $arrpeople[$num]['adults'] ? 'vbo-guestscount-complete' : 'vbo-guestscount-incomplete'; ?>" data-roomind="<?php echo $ind; ?>">
									...
									<span class="vbo-roomdet-guests-toggleword"><?php echo JText::translate('VBOGUESTSDETAILS'); ?> (<span id="vbo-guestscount-<?php echo $ind; ?>"><?php echo $count_pax_data; ?></span>/<?php echo $arrpeople[$num]['adults']; ?>)</span>
								</span>
							</div>
						</div>
					</div>
					...
					<div class="vbo-roomdet-guest-details" data-roomind="<?php echo $ind; ?>" data-totguests="<?php echo $arrpeople[$num]['adults']; ?>">
						...
						<div class="vbo-roomdet-guest-detail">
							...
							if ($paxk == 'country') {
								...
								?>
								<input type="text" autocomplete="off" data-gind="<?php echo $g; ?>" class="<?php echo $pax_field_class; ?>" name="guests[<?php echo $ind; ?>][<?php echo $g; ?>][<?php echo $paxk; ?>]" value="<?php echo (isset($current_guest[$paxk]) ? htmlspecialchars($current_guest[$paxk]) : ''); ?>" <?php echo $pax_fields_attributes[$paxk]; ?> />
								<?php
							}
							?>
							</div>
						...
						if (count($pax_data) && isset($pax_data[$ind]) && isset($pax_data[$ind][$g])) {
							foreach ($pax_data[$ind][$g] as $extrak => $extrav) {
								if (isset($pax_fields[$extrak]) || !strlen($extrav)) {
	...
	jQuery(document).ready(function() {
		...
		jQuery('input, textarea, select').keyup(function() {
			var dontupdateonly = <?php echo $today_midnight > $order['checkout'] ? 'true' : 'false'; ?>;
			if (dontupdateonly) {
				// original condition was current_checked == 0 && dontupdateonly, but we need the update button also for no-status
				return true;
			}
