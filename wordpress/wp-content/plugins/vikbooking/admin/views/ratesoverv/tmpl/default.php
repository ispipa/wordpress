...
var vboMapWdays = ["'.mb_substr(JText::translate('VBSUNDAY'), 0, 3, 'UTF-8').'", "'.mb_substr(JText::translate('VBMONDAY'), 0, 3, 'UTF-8').'", "'.mb_substr(JText::translate('VBTUESDAY'), 0, 3, 'UTF-8').'", "'.mb_substr(JText::translate('VBWEDNESDAY'), 0, 3, 'UTF-8').'", "'.mb_substr(JText::translate('VBTHURSDAY'), 0, 3, 'UTF-8').'", "'.mb_substr(JText::translate('VBFRIDAY'), 0, 3, 'UTF-8').'", "'.mb_substr(JText::translate('VBSATURDAY'), 0, 3, 'UTF-8').'"];
var vboMapMons = ["'.mb_substr(JText::translate('VBMONTHONE'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHTWO'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHTHREE'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHFOUR'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHFIVE'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHSIX'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHSEVEN'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHEIGHT'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHNINE'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHTEN'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHELEVEN'), 0, 3, 'UTF-8').'","'.mb_substr(JText::translate('VBMONTHTWELVE'), 0, 3, 'UTF-8').'"];';
				...
				<table class="vboverviewtable vbratesoverviewtable vbo-table" data-idroom="<?php echo $rid; ?>">
					...
					foreach ($roomrates[$rid] as $roomrate) {
						...
						<tr class="vbo-roverviewtablerow" id="vbo-roverw-<?php echo $roomrate['id'].'-'.$roomrate['idroom']; ?>">
							<td data-defrate="<?php echo $roomrate['cost']; ?>" data-roomname="<?php echo htmlspecialchars($roomrow['name']); ?>"><span class="vbo-rplan-name"><?php echo $roomrate['name']; ?></span></td>
						<?php
						while ($cell_count < $MAX_DAYS) {
							$style = '';
		...
		var jqxhr = jQuery.ajax({
			...
			if (res.indexOf('e4j.error') >= 0 ) {
				...
				jQuery('.vbo-calcrates-rateblock').each(function(k, v) {
					...
					var booknow = '<a href="' + base_booknow_link + '" class="btn btn-primary vbo-room-booknow-rct" target="_blank">Book Now</a>';
					elem.append(booknow);
				});
				//
			}
