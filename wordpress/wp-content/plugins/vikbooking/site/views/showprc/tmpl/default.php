							...
							for ($ch = 1; $ch <= $this->arrpeople[$num]['children']; $ch++) {
								...
								<li>
									...
									foreach ($intervals as $kintv => $intv) {
										...
										?>
										<option value="<?php echo ($kintv + 1); ?>"<?php echo ($pcent_interval === true ? ' data-pcentintvroom="'.$num.'" data-ratetype="'.$opt_suffix.'" data-pcentintv="'.floatval($intvparts[2]).'" data-ageintv="'.$intvparts[0].' - '.$intvparts[1].'"' : '') ?>><?php echo "{$intvparts[0]} - {$intvparts[1]} ({$pricestr} {$currencysymb})"; ?></option>
										<?php
									}
									?>
									</select>
