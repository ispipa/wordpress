				...
				<div class="vbo-precheckin-room-wrap <?php echo $count_pax_data >= $arrpeople[$num]['adults'] ? 'vbo-precheckin-guestscount-complete' : 'vbo-precheckin-guestscount-incomplete'; ?>">
					...
					for ($g = 1; $g <= $arrpeople[$num]['adults']; $g++) {
						...
						foreach ($pax_fields as $paxk => $paxv) {
							...
							<?php
							if (isset($pax_fields_attributes[$paxk]) && $pax_fields_attributes[$paxk] == 'country') {
								...
								<?php
							} elseif (isset($pax_fields_attributes[$paxk]) && is_array($pax_fields_attributes[$paxk])) {
								...
								foreach ($pax_fields_attributes[$paxk] as $attrk => $attrv) {
									?>
									<option value="<?php echo $attrv; ?>"<?php echo (isset($current_guest[$paxk]) && $current_guest[$paxk] == $attrv ? ' selected="selected"' : ''); ?>><?php echo $attrv; ?></option>
									<?php
								}
								?>
								</select>
