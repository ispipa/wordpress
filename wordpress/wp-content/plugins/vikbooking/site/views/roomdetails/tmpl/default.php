		...
		?>
			...
			<div id="vbdialog-overlay" style="display: none;">
				...
				<div class="vbdialog-inner vbdialog-reqinfo">
					...
					<form action="<?php echo JRoute::rewrite('index.php?option=com_vikbooking&task=reqinfo'.(!empty($pitemid) ? '&Itemid='.$pitemid : '')); ?>" method="post" onsubmit="return vboValidateReqInfo();">
						...
						<div class="vbdialog-reqinfo-formcont">
							<div class="vbdialog-reqinfo-formentry">
								...
								<input type="text" name="reqname" id="reqname" value="" placeholder="<?php echo JText::translate('VBOROOMREQINFONAME'); ?>" />
							</div>
							<div class="vbdialog-reqinfo-formentry">
								<label for="reqemail"><?php echo JText::translate('VBOROOMREQINFOEMAIL'); ?></label>
								<input type="text" name="reqemail" id="reqemail" value="<?php echo $cur_email; ?>" placeholder="<?php echo JText::translate('VBOROOMREQINFOEMAIL'); ?>" />
							</div>
							<div class="vbdialog-reqinfo-formentry">
						...
						if (count($this->terms_fields)) {
							if (!empty($this->terms_fields['poplink'])) {
								$fname = "<a href=\"" . $this->terms_fields['poplink'] . "\" id=\"vbof\" rel=\"{handler: 'iframe', size: {x: 750, y: 600}}\" target=\"_blank\" class=\"vbomodalframe\">" . JText::translate($this->terms_fields['name']) . "</a>";
							} else {
								$fname = "<label id=\"vbof\" for=\"vbof-inp\" style=\"display: inline-block;\">" . JText::translate($this->terms_fields['name']) . "</label>";
							}
							?>
			...
			function vboValidateReqInfo() {
				...
				alert('<?php echo addslashes(JText::translate('VBOROOMREQINFOMISSFIELD')); ?>');
				return false;
			}
			</script>
		<?php
			...
			<table class="<?php echo ($usepricecal === true ? 'vbcalprice' : 'vbcal'); ?>">
				<tr class="vbcaltrmonth">
					<td colspan="7" align="center" class="vbo-pref-bordercolor">
						<strong><?php echo VikBooking::sayMonth($newarr['mon'])." ".$newarr['year']; ?></strong>
					</td>
				</tr>
				<tr class="vbcaldays">
				<?php
