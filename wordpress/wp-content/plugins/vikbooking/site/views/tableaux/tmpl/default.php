...
var vbo_messages = {
	"adults": "<?php echo addslashes(JText::translate('VBFORMADULTS')); ?>",
	"children": "<?php echo addslashes(JText::translate('VBFORMCHILDREN')); ?>",
	"sprequest": "<?php echo addslashes(JText::translate('ORDER_SPREQUESTS')); ?>",
	"nominative": "<?php echo addslashes(JText::translate('VBOCUSTOMERNOMINATIVE')); ?>",
	"checkin": "<?php echo addslashes(JText::translate('VBPICKUP')); ?>",
	"checkout": "<?php echo addslashes(JText::translate('VBRETURN')); ?>",
	"rname": "<?php echo addslashes(JText::translate('VBSEARCHRESROOM')); ?>"
};
...
jQuery(document).ready(function() {
	...
	jQuery('.vbo-tableaux-booking').click(function() {
		...
		if (rname && rname.length) {
			...
			ovcont += '<div class="vbo-tableaux-bookdet-entry"><span class="vbo-tableaux-bookdet-lbl">' + vbo_messages.rname + '</span><span class="vbo-tableaux-bookdet-val">' + rname + rindex + '</span></div>';
		}
		//
		if (attrs.hasOwnProperty('tnominative')) {
			ovcont += '<div class="vbo-tableaux-bookdet-entry"><span class="vbo-tableaux-bookdet-lbl">'+vbo_messages.nominative+'</span><span class="vbo-tableaux-bookdet-val">'+attrs['tnominative']+'</span></div>';
		}
		if (attrs.hasOwnProperty('party')) {
			var guests = attrs['party'].split(';');
			ovcont += '<div class="vbo-tableaux-bookdet-entry"><span class="vbo-tableaux-bookdet-lbl">'+vbo_messages.adults+'</span><span class="vbo-tableaux-bookdet-val">'+guests[0]+'</span></div>';
			ovcont += '<div class="vbo-tableaux-bookdet-entry"><span class="vbo-tableaux-bookdet-lbl">'+vbo_messages.children+'</span><span class="vbo-tableaux-bookdet-val">'+guests[1]+'</span></div>';
		}
		if (attrs.hasOwnProperty('checkin')) {
			ovcont += '<div class="vbo-tableaux-bookdet-entry"><span class="vbo-tableaux-bookdet-lbl">'+vbo_messages.checkin+'</span><span class="vbo-tableaux-bookdet-val">'+attrs['checkin']+'</span></div>';
		}
		if (attrs.hasOwnProperty('checkout')) {
			ovcont += '<div class="vbo-tableaux-bookdet-entry"><span class="vbo-tableaux-bookdet-lbl">'+vbo_messages.checkout+'</span><span class="vbo-tableaux-bookdet-val">'+attrs['checkout']+'</span></div>';
		}
		if (attrs.hasOwnProperty('sprequest')) {
			ovcont += '<div class="vbo-tableaux-bookdet-entry"><span class="vbo-tableaux-bookdet-lbl">'+vbo_messages.sprequest+'</span><span class="vbo-tableaux-bookdet-val">'+attrs['sprequest']+'</span></div>';
		}
