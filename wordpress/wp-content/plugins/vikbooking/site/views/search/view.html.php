								...
								if (@count($arrtar) > 0) {
									...
									if (count($results) == $proomsnum) {
										...
										if (!empty($proomdetail) && array_key_exists($proomdetail, $arrtar) && $proomsnum == 1) {
											...
											//
											$mainframe->redirect(JRoute::rewrite("index.php?option=com_vikbooking&task=showprc&roomsnum=1&roomopt[]=".$proomdetail."&adults[]=".$arrpeople[1]['adults']."&children[]=".$arrpeople[1]['children']."&days=".$daysdiff."&checkin=".$first."&checkout=".$second.(!empty($ppkg_id) ? "&pkg_id=" . $ppkg_id : "").(!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
											exit;
										} else {
