									...
									if ($couponroomok) {
										...
										if ($coupontotok) {
											...
											if ($coupon['percentot'] == 1) {
												 ...
												 * @since 	1.3.5
												 */
												$coupondiscount = ($coupon['excludetaxes'] ? ($isdue - $tot_city_taxes - $tot_fees) : $isdue) * $coupon['value'] / 100;
		...
		if (strlen($psid) && strlen($pts)) {
			...
			if ($dbo->getNumRows() > 0) {
				...
				if ($rows[0]['status'] != 'confirmed' || (VikBooking::multiplePayments() && $rows[0]['paymcount'] > 0)) {
					...
					$obj = JPaymentDispatcher::getInstance('vikbooking', $payment['file'], $rows[0], $payment['params']);
					$array_result = $obj->validatePayment();
