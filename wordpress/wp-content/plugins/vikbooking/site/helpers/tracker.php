...
class VikBookingTracker
{
	...
	{
		$q = "INSERT INTO `#__vikbooking_trackings` (`dt`, `lastdt`, `fingerprint`, `ip`) VALUES (NOW(), NOW(), ".$this->dbo->quote(static::$fingerprint).", ".$this->dbo->quote($this->getIpAddress()).");";
		$this->dbo->setQuery($q);
		$this->dbo->execute();
	...
	public function closeTrack()
	{
		...
		if (!$track_closed) {
			...
			if (empty($prev_identifier_id)) {
				...
				$uatype = VikBooking::detectUserAgent(true, false);
				$uatype = !empty($uatype) ? strtoupper(substr($uatype, 0, 1)) : '';
				// insert a new tracking info record
				$q = "INSERT INTO `#__vikbooking_tracking_infos` (`idtracking`, `identifier`, `trackingdt`, `device`, `trkdata`, `checkin`, `checkout`, `idorder`, `referrer`) VALUES (
					".(int)$id_tracking.", 
					".$this->getIdentifier().", 
					NOW(), 
					".$this->dbo->quote($uatype).", 
					".$this->dbo->quote(json_encode(static::$trackdata)).", 
					".(!empty($cur_checkin) ? $this->dbo->quote($cur_checkin) : 'NULL').", 
					".(!empty($cur_checkout) ? $this->dbo->quote($cur_checkout) : 'NULL').",
				...
				$q = "UPDATE `#__vikbooking_tracking_infos` SET 
					`trackingdt`=NOW(),
					`trkdata`=".$this->dbo->quote(json_encode($new_trackdata)).", 
					`checkin`=".(!empty($cur_checkin) ? $this->dbo->quote($cur_checkin) : 'NULL').", 
					`checkout`=".(!empty($cur_checkout) ? $this->dbo->quote($cur_checkout) : 'NULL').", 
			...
			$q = "UPDATE `#__vikbooking_trackings` SET `lastdt`=NOW()".(isset(static::$trackdata->idcustomer) ? ', `idcustomer`='.(int)static::$trackdata->idcustomer : '')." WHERE `id`=".(int)$id_tracking.";";
			$this->dbo->setQuery($q);
			$this->dbo->execute();
